...
if(isset($_POST['deliverBtn'])){
    ...
    $message = $_POST['message'];
        ...
        $query = "INSERT INTO `msg` (`user_id`, `sender_name`, `date_sent`, `message`, `status`, `order_status`, `user_type`)VALUES('$user_id', '$sender_name', CURRENT_TIMESTAMP, '$message', 'unread', 'Delivered', 'admin')";
            ...
            <div class="card mb-3">
                ...
                ?>  
                          ...
                          <tbody>
                            ...
                            if($result){
                              while($res = mysqli_fetch_array($result)) {  
                                ...
                                $query3=mysqli_query($dbconn,"SELECT * FROM `order` WHERE `order_id`='$order_id' && `status`='Canceled'") or die (mysqli_error());
                             ...
                             ?>
                              ...
                              <td><br><?php echo date('F j, Y / g:i a',strtotime($row3['date_delivered']));?></td>
                      ...
                      <div class="row" style="margin-left: 50px">
                        <h3>Delivered To: </h3> <u style="margin-left: 10px; font-size: 20px;"><?php echo $row3['firstname'];?> <?php echo $row3['middlename'];?><?php echo $row3['lastname'];?></u>
                      </div>
                      <div class="row" style="margin-left: 50px">
                      ...
                      <div class="row" style="margin-left: 50px">
                        <h3>Total Payment: </h3> <u style="margin-left: 10px; font-size: 20px;">&#8369;<?php echo $row3['totalprice'];?></u>
                      </div>

                      <div class="row" style="margin-left: 50px">
                        <h3>Status: </h3> <i style="margin-left: 10px; font-size: 20px;"><span style="background: red; color: white"><?php echo $row3['status'];?></span></i>
                      </div>


  <?php
        $query=mysqli_query($dbconn,"SELECT * FROM `msg` WHERE `user_id`='".$_GET['user_id']."' && `order_status`='Canceled' && `order_id`='".$_GET['order_id']."'")or die(mysqli_error());
        while($row=mysqli_fetch_array($query)){
    ?>
                      <div class="row" style="margin-left: 50px">
                        <div class="card mb-3">
                        <h3>Message: </h3> <i style="margin-left: 10px; font-size: 20px;"><?php echo $row['message'];?></i>
                      </div>
                      </div>
  <?php } ?>
                      ...
                      <center>
                       ...
                       <!--  <button type="button" class="btn btn-primary btn-round mb-1" data-toggle="modal" data-target="#mediumModal">
                         Message Customer
                        </button> -->
                         <a class="btn btn-outline-success" href="messageCustomer.php?user_id=<?php echo $user_id?>">Message Customer</a> 
    ...
    <!-- modal medium -->
        ...
        <form id="packaging" action="viewShipping.php?order_id=<?php echo $order_id ?>&&user_id=<?php echo $user_id?>" method="post">

            <center>
              <span><b>Deliver Order</b></span>
            </center>
          ...
          <div class="form-group validate">
                <label for="textArea" class="control-label mb-1">Message</label>
                 <textarea name="message" id="textarea-input" rows="9" placeholder="Input Message Here... (Ex. Your item is now succesfully delivered. Thank you for shopping)" class="form-control" required="required"></textarea>
           </div>


             <div>
                <button  class="btn btn-success btn-block" type="submit" name="deliverBtn">Send</button>
              </div>
