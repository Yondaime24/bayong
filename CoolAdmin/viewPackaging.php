...
if(isset($_POST['deliverBtn'])){
    ...
    $message = $_POST['message'];
        ...
        $query = "INSERT INTO `msg` (`user_id`, `sender_name`, `date_sent`, `message`, `status`, `order_status`, `user_type`)VALUES('$user_id', '$sender_name', CURRENT_TIMESTAMP, '$message', 'unread', 'Shipping', 'admin')";
        $query_run = mysqli_query($dbconn, $query);

        if($query_run==1){

         $query="UPDATE `order` SET `status`='Shipping' WHERE order_id='$order_id'";
         $query_run = mysqli_query($dbconn, $query);
...
<head>
    ...
    <title>BAYONG | View Packaged</title>
...
<body class="animsition">
  <div class="page-wrapper">
                          ...
                          <tbody>
                            ...
                            if($result){
                              while($res = mysqli_fetch_array($result)) {  
                                ...
                                $query3=mysqli_query($dbconn,"SELECT * FROM `order` WHERE `order_id`='$order_id' && `status`='Packaged'") or die (mysqli_error());
    ...
    <!-- modal medium -->
        ...
        <form id="packaging" action="viewPackaging.php?order_id=<?php echo $order_id ?>&&user_id=<?php echo $user_id?>" method="post">
          ...
          <div class="form-group validate">
                <label for="textArea" class="control-label mb-1">Message</label>
                 <textarea name="message" id="textarea-input" rows="9" placeholder="Input Message Here... (Ex. Your item is now being delivered! Please prepare cash upon receiving)" class="form-control" required="required"></textarea>
           </div>
